...
Inferring two types of gene transfer using whole genomes of \emph{Streptococcus
pyogenes} and\emph{ S.\ dysgalactiae} subspecies \emph{equisimilis} 
...
\subsubsection*{Version: 0.65 (July 8, 2011) \clearpage{}}


\section*{Abstract}

Using two closely related species of \emph{Streptococcus pyogenes}
(SPY) that is a pervasive human-specific pathogen, and \emph{S.\ dysgalactiae}
ssp.\emph{\ equisimilis} (SDE) that is its closely related species
and an opportunistic human pathogen we investigated two types of gene
transfer: recombining and non-recombining
gene transfer. The former lateral process was studied
with the core genomes of \emph{Streptococcus} genomes, and the latter
process with the pan genomes as well as the core genomes. Two
complementary methods were employed to examine genes and their
fragments that could have been exchanged between SPY and SDE as efforts
of finding their roles in the formation of pathogenicity of the bacteria.
We revisited the problem of directionality of gene transfer between
SPY and SDE, and found evidence supporting that homologous
recombining gene transfer was polarized to be relatively from SPY to SDE based
on the core genomes.  Although we did not find evidence for directional
non-recombining gene transfer, virulence genes appeared to undergo
non-recombining gene transfer more than did non-virulence genes.
Recombining gene transfer was not found to be associated with
virulence genes.  This might indicate that additional genes obtained via
non-recombining gene transfer were more effective than gene fragments
transfer via bacterial recombination for SDE in the acquisition of virulence
genes.  While horizontal gene transfer studies traditionally 
focused on genes as a whole,
we rediscovered smaller gene fragments and their transfer
due to recombination between SPY and SDE in aminoacyl tRNA
synthetase (aaRS), and phosphotransferase systems (PTS).
...
\textbf{Two lateral processes of gene transfer:} 
Foreign bacterial genes can be laterally
brought into a bacterial genome by two different processes 
\citep{Ochman2001,Lawrence2009}:
homologous recombining transfer and non-recombining transfer.
In the former process donor genes that are homologous to those in
a recipient species replace the indigenous genes. In the latter process
donor genes are integrated into a recipient species without replacement
of genes in the species. 
Although the discrimination between the two
lateral processes was noted a decade ago, methods of detecting horizontal
gene transfer did not appear to properly take the differences into
account. 
Proper discrimination of the two processes of gene transfer
could lead to better understanding of biological nature
of transferred genes. For example, 
bacterial lateral transfers of small fragments of ribosomal RNA genes 
were known to be 
due to homologous recombination \citep{Sneath1993,Yap1999} not non-recombining
gene transfer. 
Because traditionally non-recombining gene transfer in bacteria has been
referred to as horizontal gene transfer, association of ribosomal RNA genes with
just lateral transfer might be misleading.
...
%\textbf{Last paragraph before methods:} 
With these
...
% \texttt{MAYBE ELABORATE MORE ON RESTRICTIONS OF CLONAL-ORIGIN'S MODEL? 
% SC:I HAVE NOT THOUGHT ABOUT MORE RESTRICTIONS ON THE RECOMBINANT TREES.}

% ClonalOrigin is applied in two stages. In both stages, sampling is performed 
% separately for the different alignment blocks. In the first stage, full 
% sampling is performed to obtain block-specific estimates of the model 
% parameters integrating out the possible recombinant trees. The block-specific 
% estimates of mutation rate, recombination rate, and average tract length are 
% then combined using a weighted median scheme to obtain a global estimate for 
% each parameter. In this scheme, each estimate is weighted according to the 
% length of the block with which it is associated, down-weighing the less 
% reliable estimates obtained from short blocks. In the second stage of 
% ClonalOrigin, the model parameters are fixed to their global estimate across 
% all blocks, and only the recombinant tree is sampled. Later this section we 
% describe several summaries of this posterior sample that are used in our 
% analysis. We used ClonalOrigin subversion r19 (downloaded from 
% \url{http://clonalorigin.googlecode.com/svn/trunk}). For the first stage, we 
% used two independent chains with 1,000,000 burn-in generations and  10,000,000 
% sampling generations, sub-sampled every 100,000 generations. The first chain 
% was used to obtain the block-specific estimates, and the second chain was used 
% to validate adequate convergence of the first. For the second stage, we used a 
% single chain with 10,000,000 burn-in generations and  100,000,000 sampling 
% generations, sub-sampled every 100,000 generations.
...
% This sampling frequency is an 
% estimate of the posterior probability taking into consideration the 
% sequence data and the setting chosen for the model parameters in the second 
% stage of ClonalOrigin. 
...
%\textbf{Simulation study for checking the surrogate measure of recombination intensity:} 
...
%\textbf{Genome sequence alignments and clonal frame:}
...
length was 4215 bp, and total length of the blocks is 1155016 bp.  There was good 
...
% \texttt{THIS FIGURE OF RECOMBINATION BOUNDARIES NEEDS TO BE REPLACED OR REMOVED
% BECAUSE I DO NOT FIND REASONS OF ADDING THIS TO THE MANUSCRIPT.}
% For further validation of the 
% estimated recombnation rates, we recorded the number of recombinant edges 
% ClonalOrigin samples that are entirely contained within a block; 
% this number 
% should correlate well with the block-specific recombination rates. Figure 
% \ref{fig:recombination-boundaries} shows the numbers obtained in our MCMC runs, 
% and indeed these numbers appear to be well correlated with the block-specific 
% recombination rates. 

%\textbf{Marginal evidence for unbalaned gene flow from SPY to SDE:}
...
%\textbf{Gene gain, loss, duplication, and non-recombining horizontal gene transfer:} 
...
the species tree.
%\textbf{No evidence for unbalanced non-recombining gene transfer between SPY and SDE:}
...
\includegraphics[scale=0.42]{figures/figure1}
...
% Figure 2. Heat maps.
\begin{figure}
\includegraphics[scale=0.45]{figures/figure2}
...
\begin{figure}
\subfloat[]{\includegraphics[scale=0.5]{figures/scatter-plot-parameter-1-out-theta}}

\subfloat[]{\includegraphics[scale=0.5]{figures/scatter-plot-parameter-1-out-rho}}

\subfloat[]{\includegraphics[scale=0.5]{figures/scatter-plot-parameter-1-out-delta}}
...
\begin{figure}
\includegraphics[scale=0.7]{figures/species}
...
site for each block along the \emph{S.\ dysgalactiae} ssp.\emph{\ equisimilis}
ATCC 12394.}
\end{figure}
\clearpage{}%
...
exploited their method to infer flows of genetic segment between \emph{Bacillus
...
related \emph{Streptococcus} species.
...
and limitations of ClonalOrigin under the parameter setup that we
...
the reverse direction \emph{a priori} (Figure S\ref{fig:tree5}). 
