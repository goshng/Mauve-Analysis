...
\section*{A list our major and minor findings or observations}
\begin{enumerate}
...
section \textbf{Results}.
\item Ilan suggested a different order of four parts in \textbf{Results}:
preliminary, funtional category and virulence gene analyses, gene gain and loss,
and directionality.
\end{enumerate}
...
gene transfer from SDE to SPY. They also \citet{Davies2009} demonstrated that an
...
Recently, a method of inferring
...
cereus} species in the core genomes.   
% Descriptions of ClonalOrigin model.
Other evolutionary processes acting on
...
transfer events was developed by Doyon et al.\ \citet{Doyon2011}.  
% Descriptions of Mowgli analsysis.
...
% easily detected using the pan genomes. 
...
the amount of used genome data. Our analysis of the core genomes also showed for the
first time that the net directionality of recombining gene transfer from SPY to
SDE was stronger at the genome scale.
...
We show a net directionality of recombining gene transfers from SPY to SDE. 
...
\subsection{Bayesian analysis of recombining gene transfer}

{\bf IG: REMOVED A LOT OF DETAIL (MAINLY IN FIRST PART). PLEASE CHECK.}

We followed the pipeline and methods suggested in \cite{Didelot2010} to pursue a model based study of recombining gene transfer. A preliminary step in this analysis involved obtaining an estimate of the species tree with branch lengths (via ClonalFrame), and estimates of the three population parameters: mutation rate, recombination rate, and average recombinant tract length (via a first phase of ClonalOrigin). Due to divergence of the SEE sample from the other five, we could not obtain suficiently long reliable multiple sequence alginments with it, and had to exclude it from the bayesian analysis. The spcies tree inferred by ClonalFrame over the five species has the expected topology (Fig. {\bf [ADD FIGURE 1]} (see Figure S\ref{fig:tree5} for branch length estimates). ClonalFrame also produces estimates for the relative strength of recombination compared
to mutations ($r/m$; Table S\ref{tab:clonalframe}).  The estimate of $r/m=4.66$ provides an initial indication to the important part recombination took in the evolutionary process of shaping these genmes: four times more sites have been affected by recombination-driven substitutions than have been affected by mutation-driven substitutions. Estimates of population mutation rate ($\theta$), recombination rate ($\rho$), and average tract length ($\delta$) were obtained for the different alignment blocks analyzed (Figure S\ref{fig:scatter3}). Global estimates of these three population parameters were obtained by a length-weighted average (Materials and Methods). The local estimates typically seem to be moderately fluctuating around the global average, with the exception of what apears to be a recombination hotspot at a region 200 kilobase pairs from the origin of replication of SDE1. 

The species tree and estimated branch lengths, together with the global estimates of the three population parameters are used in the second phase of ClonalOrigin, where recombinant trees are sampled from the posterior distriution. One way to summarize the posterior sample of recombinant trees provided by ClonalOrigin is to observe the topologies of the local genealogies implied by it. Out of the 105 possible rooted labeled topologies spanning five taxa, the dominant topology (sampled 66.7\% of the time) was that of the species tree (Materials and Methods; Table S\ref{tab:Gene-tree-topologies}). The second most frequently sampled topology (sampled 9.3\% of the time) was the one in which SPY and SDE are sister clades (with SDD as an outgroup). Other topologies were sampled with frequency less than 4\%. This gives initial indication to the extent of recombining gene transfer  between SPY and SDE.

To gain a more fine-scale view of the main types of recombination scenarios sampled by ClonalOrigin, we looked at the recombinant edges present in the sampled recombinant trees. Following the approach of \cite{Didelot2010}, we classified these edges by types defined by distinct pairs of donor and recepient branches of the species tree (Fig. [ADD FIG 1]). For each edge type, we recorded the average number of edges of that type present in the sampled recombinant tree, and computed the ratio of this observed count and the prior expected count as implied by the species tree (considering branch lengths) and the three population parameters estimated in the preliminary round of the analysis. The base 2 logarithms of these ratios are depicted in the heatmap of Figure \ref{fig:Heatmap-of-recombination} (see Tables
S\ref{tab:heatmap} and S\ref{tab:obsheatmap}).

Among the possible recombinant edge types, we distinguished between ones which preserve the species tree topology, and ones which alter it. Our main focus was on the topology-altering types, both because our main objective was to study cross-clade gene transfer, and because our simulation study indicates that the observed counts of topology-preserving recombinant edges are somewhat less reliable {\bf[IG: POINT TO RELEVANT SECTION]}. The heatmap allows us to find recombination scenarios that deviate from the expectation dictated by the probabilistic model of ClonalOrigin. {\bf [IG: CHECK THIS FOLLOWING LINE OF ARGUMENT]} We identify two main potential causes for such a deviation: (1) reduced or increased opportunities for species to interact and exchange genetic material; (2) selection acting on newly transferred genes. 

We observe a reduced count of recombinant edges sampled between SDD and the three branches of the SPY clade ($\sim$25\% of the expected count). This is consistent with the fact that these species have reduced opportunities to interact, SPY being a human pathogen and SDD being strictly veterinary. On the contrary, the number of sampled recombinant edges between SDD and branches in the SDE clade seems to be consistent with the prior, with a slight excess for edges from SDE1 and SDE2 to SDD {\bf [IG: IS THIS WORTH DWELLNG ON?]}. The greatest deviation of observed counts from expected ones was observed for recombinant edges between the SPY and SDE clades. This is consistent with the fact that these two species share an ecological niche (SDE being an opportunistic human pathogen). Interestingly, there appears to be much higher enrichment (up to four times more than expected) for recombinant edges originating in the SPY clade and ending up in the SDE clade. In a follow-up simulation study we show that this deviation from the expected count is indeed highly significant, and so is the observed asymmetry in pattern of recombination {\bf[IG: POINT TO RELEVANT SECTION]}. This bias in directionality of recombination pattern between SPY and SDE is especially intriguing in light of the hypothesis that SDE acquired human pathogenicity via gene transfer from SPY. 
...
{\bf [IG: MODIFIED THIS PARAGRAPH]}

As in the Bayesian analysis of ClonalOrigin, we attempted to see what types of transfer events (horizontal and recombining) were most common in our inference. We classified events into types defined by donor-recepient branches in the species tree. We recorded the number of inferred events of each type in Figures \ref{fig:hgt-heatmap} \&S\ref{fig:mowgli-recomb-heatmap} {\bf [IG: CONSIDER MOVING BOTH INTO SUPPLEMENT]}. Unlike in the Bayesian analysis, we cannot normalize these counts by their expectation, since we do not have any model-based expectation for them. This restrict our ability to interprest these counts. We did notice, however, a very high number of inferred recombining gene transfer events from the ancestral SPY branch to the ancestral SDE branch, consistent with our observations from the Bayesian analysis. With horizontal gene transfer, we do not see a similar pattern, but we do observe a high number of horizontal transfers from the ancesral SDE branch to SDD. {\bf [IG: IS THERE ANYTHING ELSE WE CAN SAY ABOUT THIS? MAYBE SHOW A SCATTER PLOT OF THE TWO TYPES OF EVENTS? ]}
...
We set out to seek cateories of genes that are significantly enriched for transfer events of either type (recombining and  horizontal). For horizontal transfer, we used a score based on the number of events inferred by the parsinomy-based method, and for recombining gene transfer, we used both the parsimony-based inferred counts and a recombination intensity score based on the Bayesian analysis (Materials and Methods). First, we checked association of transfers with virulence genes {\bf [IG: A SENTENCE ON WHERE WE TAKE THESE GENES FROM]}. This set of virulence genes was found to be significantly associated with horizontal gene transfers (P-value $<1.33 \times 10^{-11}$), but not with recombining gene transfers (P-value $>0.90$ for parsimony-based score and P-value $>??$ for recombnation intensity). We also found a marginally significant association between virulence genes and gene duplications (P-value $<7.13 \times 10^{-3}$).



%\subsection{Transporters, transferases, aminoacyl tRNA synthetases, and transposases}


We conducted similar tests for other functional categories of genes. Several functional categories that were found to be associated with recombination intensity were related with either membrane-bound molecules or aminoacyl tRNA synthetases (Table
\ref{tab:functional}). Consistent results were obtained for the parsimony-based counts of recombining gene transfers (Table S\ref{tab:go-events-recombining}). Functional categories associated with horizontal gene transfer were clearly different from those associated with recombining gene transfer (Table \ref{tab:go-events}). The top ranked functional categories in that list were related with transposases. These differences add to our previous findings about the different roles these two types of transfer play in the evolution of these species.


{\bf [IG: I THINK THE FOLLOWING 3 PARAGRAPHS SHOULD BE MOVED TO DISCUSSION]} 

 Molecular evolutionary forces could act on genetic materials of organisms, and changes to the genotypes might lead to some phenotypes that could change biological functions of the species. So, the bacterial gene transfer process as an evolutionary force could be associated with some biological functions. We
attempted to find which biological functions were associated with recombining or
horizontal gene transfers.  

Because bacteria as a unicellular organism should be able to respond to changes to environment, some evolutionary pressures must have been acting on genes encoding membrane-bound molecules. Recombining gene transfer process seemed to be more associated with biological functions that had been critical of sensing queues of bacterial environmental changes.  Sugar phosphotransferase classes
...
In addition to these biological categories, we also found several aminoacyl tRNA synthetases (GO Term translation GO:0006412) in association with recombining gene transfer.  In the parsimony-based approach the term translation was top
ranked (Table S\ref{tab:go-events-recombining}).  They had been a focus of gene
transfer in bacterial species \citep{Woese2000}.  

{\bf [IG: DO WE NEED/WANT THE FOLLOWING DISCUSSION? MAYBE IN MATERIALS AND METHODS - JUSTIFYING WHY WE MEASURE INTENSITY USING TOPOLOGY-CHANGING REC EDGES.]}

 Some of the functional categories were unexpected; functional categories concerning ribosomes were found to be associated with recombining gene transfer.  Because genes related with ribosomes were considered to be housekeeping genes, gene transfer in which
would be rare, it was unexpected that ribosome related genes were found to be
associated with recombining gene transfer.  Because it was a bizarre result, we
investigated the association by considering different recombinant edge types. We
noted that two different classes of recombinant edge types: one that changes
...
The similarity between the two lists of gene categories found to be associated with recombining gene transfers in the separate analyses -- Bayesian and parsimony-based --  indirectly insdicate correlation between the two approaches. In order to obtain a more direct measure of correlation, we used the set of genes present in both analyses to compute the correlation between recombination intensity scores and the number of recombinng gene transfers inferred by the parsimony-based approach. We observed a positive correlation of $0.39$ (P-value $2.2\times10^{-16}$; Figure S\ref{fig:cmpcomowgli}) between the two measures.  For comparison, the correlation between recombination intensity and the number of horizontal gene transfers was 0.03 (P-value 0.18;  \&S\ref{fig:cmpcomowglihgt}).  {\bf [IG: MAYBE THERE IS A BETTER WAY TO DEMONSTRATE THE CORRELATION BETWEEN METHODS]}.
...
{\bf [IG: CONSIDER WHAT WE WANT TO SAY HERE]}
