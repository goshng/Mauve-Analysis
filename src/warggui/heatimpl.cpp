...
void HeatImpl::compute_correct(int mode)
{
  ...
  for (int i=0; i<n; i++) for (int j=0; j<n; j++)
      {
        QString val;
        if (mode==3) val=QString::number(corr[i][j]/its);
        else if (corr[i][j]==0.0||(corr[i][j]/its<3 && states[i][j]/its<3)) val="";
        else if (mode==2) val=QString::number(states[i][j]/corr[i][j]);
...
void HeatImpl::drawGraph(QPaintDevice * qpd)
{
  ...
  for (int i=0; i<n; i+=5)
    {
      painter.drawLine(QLineF(1.0*i/n,0.0,1.0*i/n,1.0));
