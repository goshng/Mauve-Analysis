...
RecTree::RecTree(Data * data,double rho,double delta,vector<int> blocks):Tree(data)
{
    ...
    edge=vector<RecEdge*>(0);
    if(rho==0.0) return;
    else dropEdges(rho,delta,blocks);
}
...
/**
 * Sang adds a constructor to create a recombinant tree with a given species
 * tree.
 * Note that Tree has a data member, n.
 */
RecTree::RecTree(string newickTree,
                 double rho,
                 double delta,
                 vector<int> blocks) : Tree(newickTree, false, false)
{
    tabNode=vector<int>(n*2-1);
    tabRec=vector<int>(MAXNODES);
    tabSons=vector<vector<int> >(MAXNODES,vector<int>(2,0));
    ...
    while(1) {
        ...
        if(res.find("<recedge>")!=string::npos) {
            try {
                ...
                cerr<<"Error in reading XML file:"<<x<<endl;
                exit(0);
            }
        }
...
double RecTree::priorEdge(int e,Param * param) const
{
    ...
    if (xatborder) ret+=log(delta/(b*delta+length-b));
    else ret+=log(1.0/(b*delta+length-b));
    if (yatborder) ret+=(y-x+1)*log(1.0-1.0/delta);
    else ret+=(y-x)*log(1.0-1.0/delta)-log(delta);
    return(ret);
}
...
double RecTree::prior(Param * param) const
{
    ...
    if (param->getRho()>0.0) {
        double T=getTTotal();
        ret+=-param->getRho()*T*0.5+edge.size()*log(param->getRho()*0.5);
    }
    else if (edge.size()>0) ret=-INFINITY;
...
#if defined DEBUG
    try {
        testNodeAges(); //test its ok so far
    } catch(char * x) {
        dlog(1)<<x<<endl<<"Failed to move node "<<which<<" to "<<*whichto<<endl;
        for(int i=0; i<2*getN()-1; i++) {
            dlog(1)<<"Age["<<i<<"]="<<getNode(i)->getAge()<<endl;
        };
        exit(1);
    }
