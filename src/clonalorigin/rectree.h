/** \file rectree.h
 * A recombinant tree is implemented by RecTree.
 */
#ifndef RECTREE_H
#define RECTREE_H
#include "tree.h"
...
// Constructors and destructors:
    ...
    /**
     * Add recombinant edges to a tree by simulation. 
     * ClonalOrigin was implemented with a method of simulating data under its
     * model. One of the constructors creates a recombinant tree given a number
     * of taxa. A coalescent tree is simulated by Class Tree with the number of
     * taxa. Sang Chul replaces the contructor of the base class Class Tree with
     * one with argument of a newick formatted string. The following is the
     * prototype of the original constructor on which the following constructor
     * is based.
     * \code
     * RecTree(int n,double rho,double delta,std::vector<int> blocks);
     * \endcode
     */
    RecTree(string newick, double rho, double delta, std::vector<int> blocks);
