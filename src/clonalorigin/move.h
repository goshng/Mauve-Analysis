...
public:
    ...
    virtual inline int getTopoCounts() {
        return(-1);
    };
    virtual inline int getTopoAcc() {
        return(-1);
    };
    virtual inline int move(vector<int> *samplespace) {
        cout<<"Problem in Move!"<<endl;
        return(move());
    };///<Performs the move once
