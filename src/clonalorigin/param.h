...
public:
    ...
    inline double hyperPriorOfRho() {
        return(hRho);   ///<The hyperprior for rho p(rho)=hRho*exp(-rho*hRho), i.e. it is a rate parameter
    }
    ...
    inline void setTempering(double t) {
        tempering=t;   ///< Sets the tempering value
    }
    inline double getTempering() {
        return(tempering);   ///< Sets the tempering value
    }
