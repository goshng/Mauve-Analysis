                    ...
                    if(rectree->getEdge(c1)->getEdgeTo()==r1[c2] || rectree->getEdge(c1)->getEdgeTo()==s1)
                        {
                            for(int c3=0; c3<lcaindexr2; c3++)
                                {
                                    if(rectree->getEdge(c1)->getEdgeFrom()==r2[c3] || rectree->getEdge(c1)->getEdgeFrom()==s2)
                                        {
                                            reccounted++;
                                            c3=lcaindexr2;
                                            c2=lcaindexr1;
...
// go through every pair of sequences
                    for(unsigned int c2=0; c2<slist.size(); c2++)
                        {
                            for(unsigned int c3=0; c3<slist[c2].size(); c3++)
                                {
...
void MoveGreedyTree::applyChanges(vector<double> newage)
{
    ...
    if(map.size()!=newage.size())
        {
            cerr<<"MoveGreedyTree::applyChanges Error: List of ages of size "<<newage.size()<<" but need size "<<map.size()<<endl;
            throw("MoveGreedyTree Error: Age size incorrect");
        }
