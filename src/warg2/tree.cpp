                    ...
                    if (*dist>nodes[i]->getDist()) *dist-=nodes[i++]->getDist();
                    else
                        {
                            if (samplespace==NULL) ans = i;
                                ...
                                {
                                    for (unsigned int j=0; j<samplespace->size(); j++) if (samplespace->at(j)==i)
                                            {
                                                ans=i;
                                                j=samplespace->size();
                                            }
    ...
    while (toCluster.size()>1)
        {
            ...
            for (unsigned int i=0; i<toCluster.size(); i++) for (unsigned int j=i+1; j<toCluster.size(); j++)
                    if (dist[toCluster[i]][toCluster[j]]<mini || a==-1)
                        {
                            a=i;
                            b=j;
                            mini=dist[toCluster[i]][toCluster[j]];
                        };
            //Merge two clusters
