...
void Data::subset(vector<int> numv,int seed)
{
    ...
    if (num>getB() &&seed>=0)
        {
            cerr<<"Not enough regions for subset, ignoring argument."<<endl;
            return;
        }
    ...
    if(seed<0)
        {
            for(int i=0; i<numv.size(); i++) if(numv[i]<tried.size()&&numv[i]>=0)
                    {
                        if(!tried[numv[i]])
                            {
                                regions.push_back(numv[i]);
                                tried[numv[i]]=true;
                            }
                        else
                            {
                                cerr<<"Warning: Duplicate blocks of "<<numv[i]<<" were specified and have been removed from the analysis."<<endl;
                            }
                    }
                else
                    {
                        cerr<<"Warning: Invalid blocks were specified and have been removed from the analysis."<<endl;
                    }
            num=regions.size();
        }
    else
