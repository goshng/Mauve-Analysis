...
# set before running any simulation-based analysis.
function prepare-filesystem-simulation {
  # The main base directory contains all the subdirectories.
  MAUVEANALYSISDIR=`pwd`
  ...
  # at the same time. Note that you have to change many parts of the codes if
  # the cluster's submission system is different from Cornell CAC Linux cluster.
  BATCH_SH_RUN_CLONALORIGIN=$BASECHOICEDIR/batch.sh
  BATCH_BODY_SH_RUN_CLONALORIGIN=$BASECHOICEDIR/batch_body.sh
  BATCH_TASK_SH_RUN_CLONALORIGIN=$BASECHOICEDIR/batch_task.sh
